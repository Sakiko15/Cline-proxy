...
在 VPS 上用 1Panel 的「编排」功能部署 Cline-proxy：VPS 上拉取源码，1Panel 自动构建镜像并启动，之后更新只需 `git pull` + 重新部署。

## 1. 前置条件

- VPS（建议 ≥1 核 1G，构建时需要少量内存）
- 已安装 1Panel（安装时默认装好 Docker）
- 云厂商安全组已放行要用到的端口（3457 直连，或 80/443 走反代）

## 2. 拉取源码

打开 1Panel 的「终端」（或 SSH 登录），执行：
...
> `touch override.md` 很重要：compose 把宿主机 `./override.md` 挂载进容器（`:ro`）。如果宿主机上这个文件不存在，Docker 会创建一个**目录**顶上去。先 touch 成空文件即可（override.md 是可选功能，内容为空时自动使用客户端自带系统提示词）。

## 3. 创建编排

1Panel 左侧：**容器 → 编排 → 创建编排**

- 来源选择 **「路径选择」**，选中 `/opt/Cline-proxy` 目录（1Panel 自动读取其中的 `docker-compose.yml`）
- 确认后点**创建**，再点**部署**，等待镜像构建与容器启动

> 为什么用「路径选择」而不是「编辑」：compose 里的 `build: .` 和 `./data`、`./override.md` 相对路径都以 compose 文件所在目录为基准。路径选择指向源码目录，保证构建上下文和数据目录都落在 `/opt/Cline-proxy` 下，数据好找、更新方便。
...
> 已核实：1Panel 编排的"重新部署"只是 `docker compose up -d`，**不会自动重建已存在的镜像**（`build: .` 只在镜像不存在时构建）。所以更新代码后必须**先手动构建**，再重启容器：
...
或在 1Panel **容器 → 镜像** 里删除旧镜像后，回编排详情页重新部署（也会触发重建）。
...
| 数据目录在哪 | `/opt/Cline-proxy/data/`（账号、API Key、日志、统计都在里面） |

## 数据与配置文件一览

| 路径（容器内） | 宿主机 | 说明 |
|------|------|------|
| `/app/data/` | `/opt/Cline-proxy/data/` | 账号池、API Key、`zen-stats.jsonl`、`requests.jsonl`、日志——**全部持久化** |
| `/app/override.md` | `/opt/Cline-proxy/override.md` | 可选：系统提示词覆盖（空文件 = 不生效） |
